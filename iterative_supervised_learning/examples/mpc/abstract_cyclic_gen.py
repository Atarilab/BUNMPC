...
class SoloMpcGaitGen:
    ...
    def update_gait_params(self, weight_abstract, t, ik_hor_ratio = 0.5, horizon=None):
        """
        Updates the gaits
        Input:
        ...
        if np.isscalar(self.params.cent_wt[0]):
            self.kd.set_com_tracking_weight(np.array([self.params.cent_wt[0]]))
            self.kd.set_mom_tracking_weight(np.array([self.params.cent_wt[1]]))
        else:
            self.kd.set_com_tracking_weight(self.params.cent_wt[0])
            self.kd.set_mom_tracking_weight(self.params.cent_wt[1])
    ...
    def create_cnt_plan(self, q, v, t, v_des, w_des, noise_std=None, mcts_x_y_cnt_loc=None, ee_pos=None, z_height=None):
        if z_height is None:
                    ...
                    if self.gait_planner.get_phase(ft, j) == 1:
                        ...
                        # if eef just got into contact with ground...
                            ...
                            # desired contact loc given
                                ...
                                start_ind = int(t / self.params.gait_dt)
                ...
                if i == 0:
                    ...
                    if self.gait_planner.get_phase(ft, j) == 1:
                        ...
                        if self.cnt_plan[i-1][j][0] == 1:
                            ...
                            if mcts_x_y_cnt_loc is None:
                                hip_loc = com + np.matmul(R, self.offsets[j])[0:2] + i*self.params.gait_dt*v_des_feet[j][0:2]
                                raibert_step = 0.5*v_des_feet[j][0:2]*self.params.gait_period*self.params.stance_percent[j] - 0.05*(vtrack-v_des[0:2])
    ...
    def optimize(self, q, v, t, v_des, w_des, X_wm = None, F_wm = None, P_wm = None, noise_std=None,
                mcts_x_y_cnt_loc=None, v_feet_des=None, ee_pos=None, z_height=None):
        ...
        if v_feet_des is not None:
            ...
            self.create_cnt_plan(q, v, t, v_des, w_des, noise_std=noise_std,
