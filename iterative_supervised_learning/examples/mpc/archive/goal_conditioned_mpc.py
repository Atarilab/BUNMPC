...
class SoloMpcGC:
    ...
    def update_gait_params(self, gait_params, t, des_cnt_plan, des_swing_time, ik_hor_ratio = 0.5, horizon=None):
        """
        Updates the gaits
        Input:
        ...
        # self.gait_planner = GaitPlanner(self.gait_params.gait_period, np.array(self.gait_params.stance_percent), \
        #                                 np.array(self.gait_params.phase_offset), self.gait_params.step_ht)
        #Different horizon parameterizations; only self.gait_params.gait_horizon works for now
        self.gait_horizon = self.gait_params.gait_horizon
        if horizon is not None:
            self.horizon = horizon
        else:
            self.horizon = int(np.round(self.gait_params.gait_horizon*self.gait_params.gait_period/self.gait_params.gait_dt,2))

        # --- Set up Inverse Kinematics ---
        self.ik_horizon = int(np.round(ik_hor_ratio*self.gait_params.gait_horizon*self.gait_params.gait_period/self.gait_params.gait_dt, 2))
        
        self.dt_arr = np.zeros(self.horizon)

        # kino dyn
        self.kd = KinoDynMP(self.r_urdf, self.m, len(self.eff_names), self.horizon, self.ik_horizon)
        #self.kd.set_com_tracking_weight(self.gait_params.cent_wt[0])
        #self.kd.set_mom_tracking_weight(self.gait_params.cent_wt[1])
        if np.isscalar(self.gait_params.cent_wt[0]):
            self.kd.set_com_tracking_weight(np.array([self.gait_params.cent_wt[0]]))
            self.kd.set_mom_tracking_weight(np.array([self.gait_params.cent_wt[1]]))
        else:
            self.kd.set_com_tracking_weight(self.gait_params.cent_wt[0])
            self.kd.set_mom_tracking_weight(self.gait_params.cent_wt[1])
        ...
        self.mp.set_rho(self.gait_params.rho)

        # --- Set up other variables ---
        self.X_nom = np.zeros((9*self.horizon))
        # For interpolation (should be moved to the controller)
        self.size = min(self.ik_horizon, int(self.planning_time/self.gait_params.gait_dt) + 2)
        if self.planning_time > self.gait_params.gait_dt:
            self.size -= 1
    ...
    # def create_cnt_plan(self, q, v, t, v_des, w_des, sim_time, noise_std=None, mcts_x_y_cnt_loc=None, ee_pos=None, z_height=None):
    #     if z_height is None:
    #         pin.forwardKinematics(self.rmodel, self.rdata, q, v)
    #         pin.updateFramePlacements(self.rmodel, self.rdata)
    #         com = pin.centerOfMass(self.rmodel, self.rdata, q)
    #         com = np.round(pin.centerOfMass(self.rmodel, self.rdata, q, v)[0:2], 3)
    #         z_height = pin.centerOfMass(self.rmodel, self.rdata, q, v)[2]
    #     else:
    #         com = np.round(z_height[0:2], 3)
    #         z_height = z_height[2]
    ...
    #     # if mcts_x_y_cnt_loc is None:
    #     # NOTE: Loop across planning horizon
    #     for i in range(self.horizon):
            
    #         # NOTE: Loop across eef
    #         for j in range(len(self.eff_names)):
                
    #             #max_time_ind = np.argmax(mcts_x_y_cnt_loc[2, :, 0])
                
    #             #ft = np.round(t + i*self.gait_params.gait_dt,3)
    #             #if j==0:
    #             #    print(f'i:{i} | j:{j} | phase: {self.gait_planner.get_phase(ft, j)}')
    #             #print(f'stance_percent: {self.gait_planner.stance_percent_}')
                
    #             # if current planning timestep is 0
    #             if i == 0:
    #                 # check if the current eef is in the starting phase (in contact with ground)
    #                 if self.gait_planner.get_phase(t, j) == 1:
                        
    #                     # eef in contact with ground
    #                     self.cnt_plan[i][j][0] = 1
                        
    #                     # if no mcts_x_y and current ee_pos is given
    #                     if mcts_x_y_cnt_loc is None and ee_pos is None:
    #                         # start the cnt_plan with the current position of the end effectors
    #                         self.cnt_plan[i][j][1:4] = np.round(self.rdata.oMf[self.ee_frame_id[j]].translation, 3)
                        
    #                     # if ee_pos is given   
    #                     elif ee_pos is not None:
    #                         # start the cnt_plan with the given eef positions
    #                         self.cnt_plan[i][j][1:4] = ee_pos[j]
                            
    #                         #print(f'diff: {np.round(self.rdata.oMf[self.ee_frame_id[j]].translation, 3)-ee_pos[j]}')
    #                     else:
    #                         # start the cnt_plan with the current position of the end effectors
    #                         self.cnt_plan[i][j][1:4] = np.round(self.rdata.oMf[self.ee_frame_id[j]].translation, 3)
                            
    #                         #self.cnt_plan[i][j][1:3] = mcts_x_y_cnt_loc[i][j]
    #                         # self.cnt_plan[i][j][1:3] = mcts_x_y_cnt_loc[i][j][1:3]
    #                         # self.cnt_plan[i][j][3] = np.round(self.rdata.oMf[self.ee_frame_id[j]].translation, 3)[2]
                        
    #                     # save current cnt_plan in prev cnt plan
    #                     self.prev_cnt[j] = self.cnt_plan[i][j][1:4]
                    
    #                 # if current eef is not at the start of the gait...
    #                 else:
    #                     # set eef as not in contact with the ground
    ...
    #             # if current planning timestep is not 0
    #             else:
    #                 # get current planning time
    #                 ft = np.round(t + i*self.gait_params.gait_dt, 3)

    #                 # check if gait is completed at this time step
    #                 if self.gait_planner.get_phase(ft, j) == 1:
                        
    #                     # gait is completed for this eef and is in contact with ground
    #                     self.cnt_plan[i][j][0] = 1

    #                     # if eef is also in contact with ground in the prev. planning timestep...
    #                     if self.cnt_plan[i-1][j][0] == 1:
    #                         # eef theoretically shouldnt move. contact location same as prev. timestep
    #                         self.cnt_plan[i][j][1:4] = self.cnt_plan[i-1][j][1:4]
                            
    #                     # if eef just got into contact with ground...
    #                     else:
    #                         # if no contact loc xy is given...
    #                         if mcts_x_y_cnt_loc is None:
                                
    #                             # get projected hip/shoulder location
    #                             hip_loc = com + np.matmul(R, self.offsets[j])[0:2] + i*self.gait_params.gait_dt*vtrack
                                
    #                             # get Raibert heuristic -> legs landing and leaving angle should be identical if robot is travelling with v_des
    #                             raibert_step = 0.5*vtrack*self.gait_params.gait_period*self.gait_params.stance_percent[j] - 0.05*(vtrack - v_des[0:2])
    ...
    #                             start_ind = int(t / self.gait_params.gait_dt)

    #                             diff_raibert_mcts = raibert_step[0:2] + hip_loc + ang_step[0:2] - mcts_x_y_cnt_loc[i+start_ind][j][1:3]
    #                             print(f'diff_cnt_pln_raibert_mcts: {diff_raibert_mcts}')
                                
    #                             #idx_2nd_stance_in_plan = -1 #np.argmax(self.gg.cnt_plan[:i,0,0])
    #                             """
    #                             num_switch = 0
    #                             for plan_t in range(i):
    #                                 # switch stance to swing phase
    #                                 if plan_t>0 and self.gg.cnt_plan[plan_t, 0, 0] == 0 and self.gg.cnt_plan[plan_t-1, 0, 0] ==1:
    #                                     idx_2nd_stance_in_plan = plan_t+np.argmax(self.gg.cnt_plan[plan_t:,0,0])
    #                                     break
                                
    #                             if idx_2nd_stance_in_plan>=0 and :
    #                                 self.cnt_plan[i][j][1:3] = mcts_x_y_cnt_loc[i][j]
    #                             else:
    #                             """
    #                             self.cnt_plan[i][j][1:3] = raibert_step[0:2] + hip_loc + raibert_step[0:2] # mcts_x_y_cnt_loc[i][j]
    #                             #self.cnt_plan[i][j][1:3] = mcts_x_y_cnt_loc[i][j][1:3]
                                
    #                             # ind_of_current_sim_time = np.where(np.round(mcts_x_y_cnt_loc[j, :, 0],3)==sim_time)[0]
    #                             # if len(ind_of_current_sim_time) != 0:
    #                             #     if ind_of_current_sim_time[0] < max_time_ind:
    #                             #         self.cnt_plan[i][j][1:3] = mcts_x_y_cnt_loc[j, ind_of_current_sim_time[0], 1:]
    #                             #     else:
    #                             #         pass
                                
    #                             # self.cnt_plan[i][j][1:3] = mcts_x_y_cnt_loc[j, 0,1:3]
    ...
    #                 # gait is still not completed at this timestep
    #                 else:
    #                     # set foot in contact to false
    #                     self.cnt_plan[i][j][0] = 0
    ...
    #                         else:
    #                             raibert_step = 0.5*vtrack*self.gait_params.gait_period*self.gait_params.stance_percent[j] - 0.05*(vtrack - v_des[0:2])
    #                             self.cnt_plan[i][j][1:3] = hip_loc + ang_step[0:2] + raibert_step[0:2]
    ...
    #         for j in range(len(self.eff_names)):
    #             if noise_std is not None and i>0:
    #                 # As long as the foot is in contact, don't change its desired contact location
    #                 # Change it when it passes from a contact phase to a stance phase
    #                 if self.cnt_plan[i-1][j][0] == 1 and self.cnt_plan[i][j][0]==0: # or \
    #                     #(self.cnt_plan[i-1][j][0]==0 and self.cnt_plan[i][j][0]==0):
    #                     # add random noise to the (x,y) coordinates of the contact location
    #                     #print("\n norm cnt loc", np.linalg.norm(self.cnt_plan[i][j][1:3]))
    #                     self.cnt_plan[i][j][1:3] = self.cnt_plan[i][j][1:3] + np.linalg.norm(self.cnt_plan[i][j][1:3])*noise_std[i][j][1:3]
    #         if i == 0:
    #             dt = self.gait_params.gait_dt - np.round(np.remainder(t,self.gait_params.gait_dt),2)
    #             if dt == 0:
    #                 dt = self.gait_params.gait_dt
    #         else:
    #             dt = self.gait_params.gait_dt
    ...
    #     #     for i in range(self.horizon):
    #     #         for j in range(len(self.eff_names)):
    #     #             self.cnt_plan[i, j] = mcts_x_y_cnt_loc[i + start_ind, j]
    ...
    #     # print("time:", t)
    #     # print(self.cnt_plan[0][0])
    #     return self.cnt_plan
    
    # def create_cnt_plan_bis(self, q, v, t, v_des, w_des, v_des_feet, ee_pos=None, z_height=None, mcts_x_y_cnt_loc=None, noise_std=None):
    #     #pin.forwardKinematics(self.rmodel, self.rdata, q, v)
    #     #pin.updateFramePlacements(self.rmodel, self.rdata)
    ...
    #                     else:
    #                         if mcts_x_y_cnt_loc is None:
    #                             hip_loc = com + np.matmul(R, self.offsets[j])[0:2] + i*self.gait_params.gait_dt*v_des_feet[j][0:2]
    #                             raibert_step = 0.5*v_des_feet[j][0:2]*self.gait_params.gait_period*self.gait_params.stance_percent[j] - 0.05*(vtrack-v_des[0:2])
    #                             ang_step = 0.5*np.sqrt(z_height/self.gravity)*v_des_feet[j][0:2]
    #                             ang_step = np.cross(ang_step, [0.0, 0.0, w_des])
    #                             self.cnt_plan[i][j][1:3] = raibert_step[0:2] + hip_loc + ang_step[0:2]
    #                         else:
    #                             self.cnt_plan[i][j][1:3] = mcts_x_y_cnt_loc[0][j][1:3]
    ...
    #         for j in range(len(self.eff_names)):
    #             if noise_std is not None and i>5:
    #                 # As long as the foot is in contact, don't change its desired contact location
    #                 if self.cnt_plan[i-1][j][0] == 0 and self.cnt_plan[i][j][0]==1: # or \
    #                     noise_j[j] = noise_std[i,j,:]*np.linalg.norm(self.cnt_plan[i][j][1:3])
    #                     self.cnt_plan[i][j][1:3] = self.cnt_plan[i][j][1:3] + noise_j[j]
    #                 elif self.cnt_plan[i][j][0]==1:
    #                     self.cnt_plan[i][j][1:3] = self.cnt_plan[i][j][1:3] + noise_j[j]
    #         if i == 0:
    #             dt = self.gait_params.gait_dt - np.round(np.remainder(t,self.gait_params.gait_dt),2)
    #             if dt == 0:
    ...
    def create_costs(self, q, v, v_des, w_des, ori_des):
        ...
        for i in range(self.ik_horizon):
            for j in range(len(self.eff_names)):
                if self.cnt_plan[i][j][0] == 1:
                    self.ik.add_position_tracking_task_single(self.ee_frame_id[j], self.cnt_plan[i][j][1:4], self.gait_params.swing_wt[0],
                                                              "cnt_" + str(0) + self.eff_names[j], i)
                elif self.swing_time[i][j] == 1:
                    pos = self.cnt_plan[i][j][1:4].copy()
                    pos[2] = self.gait_params.step_ht
                    self.ik.add_position_tracking_task_single(self.ee_frame_id[j], pos, self.gait_params.swing_wt[1],
                                                              "via_" + str(0) + self.eff_names[j], i)

        self.ik.add_state_regularization_cost(0, self.ik_horizon, self.gait_params.reg_wt[0], "xReg", self.gait_params.state_wt, self.x_reg, False)
        self.ik.add_ctrl_regularization_cost(0, self.ik_horizon, self.gait_params.reg_wt[1], "uReg", self.gait_params.ctrl_wt, np.zeros(self.rmodel.nv), False)

        self.ik.add_state_regularization_cost(0, self.ik_horizon, self.gait_params.reg_wt[0], "xReg", self.gait_params.state_wt, self.x_reg, True)
        self.ik.add_ctrl_regularization_cost(0, self.ik_horizon, self.gait_params.reg_wt[1], "uReg", self.gait_params.ctrl_wt, np.zeros(self.rmodel.nv), True)
        ...
        self.X_nom[2::9] = self.gait_params.nom_ht
        ...
        X_ter[0:2] = self.X_init[0:2] + (self.gait_params.gait_horizon*self.gait_params.gait_period*v_des)[0:2] #Changed this
        X_ter[2] = self.gait_params.nom_ht
        X_ter[3:6] = v_des
        X_ter[6:] = amom

        self.X_nom[6::9] = amom[0]*self.gait_params.ori_correction[0]
        self.X_nom[7::9] = amom[1]*self.gait_params.ori_correction[1]

        if w_des == 0:
            self.X_nom[8::9] = amom[2]*self.gait_params.ori_correction[2]
        else:
        ...
        self.mp.create_cost_X(np.tile(self.gait_params.W_X, self.horizon), self.gait_params.W_X_ter, X_ter, self.X_nom)
        self.mp.create_cost_F(np.tile(self.gait_params.W_F, self.horizon))
    ...
    def optimize(self, q, v, sim_time, v_des, w_des, sim_step, X_wm = None, 
                 F_wm = None, P_wm = None, noise_std=None, mcts_x_y_cnt_loc=None, 
                 v_feet_des=None, ee_pos=None, z_height=None):

        # resetting origin (will otherwise cause scaling issues in the long run - Khadiv, 28 Feb 2024)
        q[0:2] = 0
        ...
        # TODO: define cnt_plan from desired cnt plan
        self.cnt_plan = np.zeros((self.horizon, len(self.eff_names), 4))
        self.swing_time = np.zeros((self.horizon, len(self.eff_names)))
        start_idx = int(sim_time / self.gait_params.gait_dt)
        
        self.cnt_plan[:] = self.des_cnt_plan[start_idx : self.horizon + start_idx]
        self.swing_time[:] = self.des_swing_time[start_idx : self.horizon + start_idx]
        
        for i in range(self.horizon):
            if i == 0:
                dt = self.gait_params.gait_dt - np.round(np.remainder(sim_time, self.gait_params.gait_dt),2)
                if dt == 0:
                    dt = self.gait_params.gait_dt
            else:
                dt = self.gait_params.gait_dt
        ...
        #TODO: Move to C++
        # t1 = time.time()
        # if v_feet_des is not None:
        #     for j in range(self.n_eff):
        #         v_feet_des[j] = np.matmul(R, v_feet_des[j])
        #     self.create_cnt_plan_bis(q, v, t, v_des, w_des, v_des_feet=v_feet_des,
        #                             ee_pos=ee_pos, z_height=z_height, noise_std=noise_std)
        # else:
        #     self.create_cnt_plan(q, v, t, v_des, w_des, sim_time, noise_std=noise_std,
        #         mcts_x_y_cnt_loc=mcts_x_y_cnt_loc, ee_pos=ee_pos, z_height=z_height)
