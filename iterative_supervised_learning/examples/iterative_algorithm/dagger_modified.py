...
class Dagger():  
    ...
    def __init__(self, cfg):        
        ...
        # Data Collection
        self.num_iterations_dagger = cfg.num_iterations_dagger
        self.expert_influence_decay = cfg.expert_influence_decay
        ...
        # MPC expert 'relabelling'
        self.num_samples_per_gait_cycle_data = cfg.num_samples_per_gait_cycle_data
        self.mpc_episode_length_data = cfg.mpc_episode_length_data
        ...
        self.vc_input_size = self.n_state + 5  # phi, vx, vy, w, gait
        ...
        save_path_base = "/dagger/" + str_gaits
        if cfg.suffix != '':
    ...
    def run(self): 
        ...
        # NOTE: main dagger loop
        for i_main in range(self.num_iterations_dagger):
            ...
            # wandb.log({'database_size': len(self.database)})
            self.database.set_goal_type('vc')
            
            if i_main == 0:  # warmup (different epoch!)
            ...
            for i_rollout in range(self.num_rollouts_per_iteration_data):
                ...
                for i_replan in replanning_list:
                    ...
                    for i_pertubation in range(self.num_pertubations_per_replanning_data):
                        ...
                        ### perform rollout
                        print("=== Policy Rollout with pertubation - ", str(i_main+1), '_', str(i_replan+1), '_', str(i_pertubation+1), " ===")
                        
                        # TODO: set mpc_usage_percentage to decaying
                        # MPC influence decay
                        mpc_usage_percentage = round(self.expert_influence_decay ** i_main, 3)
                        print('mpc_usage_percentage: ' + str(mpc_usage_percentage))
                        
                        self.database.set_goal_type('vc')
                        policy_state, policy_action, policy_vc_goal, policy_base, q_history, v_history, mpc_usage, frames = \
                            self.simulation.rollout_dagger(self.episode_length_data, start_time, v_des, w_des, gait, 
                                                            self.vc_network, des_goal=desired_goal, q0=new_q0, v0=new_v0, 
                                                            norm_policy_input=self.database.get_database_mean_std(), 
                                                            mpc_usage_percentage=mpc_usage_percentage, save_video=True)
                        ...
                        print('Policy rollout completed. No. Datapoints: ' + str(len(q_history)))     
                        wandb.finish()

                        # check if q length is longer than 0
                        if len(q_history) > 0:
                            # get number of gaits cycles for the rollouts
                            num_gait_cycle = int(len(q_history) * self.sim_dt / self.simulation.gait_params.gait_period)
                            
                            sample_list = [0]  # always do the initial position
                            for gait_cycle in range(num_gait_cycle):
                                for _ in range(self.num_samples_per_gait_cycle_data):
                                    sample_list.append(np.random.randint(gait_cycle * self.simulation.gait_params.gait_period / self.sim_dt,
                                                                         (gait_cycle + 1) * self.simulation.gait_params.gait_period / self.sim_dt))
                            
                            for sample in sample_list:
                                print("=== MPC Rollout from Policy Trajectory timestep ", str(sample), " ===")
                                
                                # rollout mpc
                                mpc_state, mpc_action, mpc_vc_goal, mpc_cc_goal, mpc_base, _ = self.simulation.rollout_mpc(self.mpc_episode_length_data + sample, sample*self.sim_dt, v_des, w_des, gait, 
                                                                                                                        q0=q_history[sample], v0=v_history[sample])
                            ...
                            # policy rollout is too short! retry
                            print('policy rollout was too short!')

                
                # record percentage of successful policy rollouts         
                wandb.init(project=project_name, config={'database_size':len(self.database), 'iteration':i_main+1, 'gait':gait}, job_type='data_collection', 
...
@hydra.main(config_path='cfgs', config_name='dagger_modified_config')
def main(cfg):
    icc = Dagger(cfg)
    # icc.warmup()
